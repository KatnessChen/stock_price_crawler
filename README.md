...
```bash
# For the dev (default) environment
...
```bash
stock-price-crawler$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
stock-price-crawler$ sam local start-api
stock-price-crawler$ curl http://localhost:3000/
```
...
```bash
stock-price-crawler$ sam logs -n HelloWorldFunction --stack-name stock-price-crawler --tail
```
...
Tests are defined in the `src/tests` folder in this project. Use NPM to install the [Mocha test framework](https://mochajs.org/) and run unit tests from your local machine.

```bash
stock-price-crawler$ cd src
src$ npm install
src$ npm run test
```
...
```bash
sam delete --stack-name stock-price-crawler
```
